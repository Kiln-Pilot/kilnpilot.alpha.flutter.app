...
class ClinkerQualityBatchSectionWidget extends StatelessWidget {
  final PlatformFile? selectedFile;
  final VoidCallback onPick;
  final Map<String, Map<String, String>> featuresMeta;

  const ClinkerQualityBatchSectionWidget({
    super.key,
        ...
        Row(
          children: [
            ...
            if (selectedFile != null)
              Expanded(child: Text('Selected: ${selectedFile!.name}', overflow: TextOverflow.ellipsis)),
          ],
        ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                SizedBox(height: 12),
                Text(
                  '${leftKeys.length + middleKeys.length + rightKeys.length} Required attributes',
                  style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.w600),
                ),
                const SizedBox(height: 16),
                // Four-column layout: three attribute columns + sample header column
                Row(
                  spacing: 12,
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        spacing: 8,
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: leftKeys.map((k) => buildItem(k)).toList(),
                      ),
                    ),
                    // Column 2
                    Expanded(
                      child: Column(
                        spacing: 8,
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: middleKeys.map((k) => buildItem(k)).toList(),
                      ),
                    ),
                    // Column 3
                    Expanded(
                      child: Column(
                        spacing: 8,
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: rightKeys.map((k) => buildItem(k)).toList(),
                      ),
                    ),
                    // Column 4: sample header and bullet list inside white container
                    Expanded(
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                                ...
                                IconButton(
                                  onPressed: () {
                                    ...
                                    ScaffoldMessenger.of(
                                      context,
                                    ).showSnackBar(const SnackBar(content: Text('CSV header copied to clipboard')));
                                  },
                            ...
                            Text('Header:', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600)),
                                    ...
                                    return Padding(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          Expanded(child: Text(k, style: GoogleFonts.poppins(fontSize: 13))),
                                        ],
                                      ),
                                    );
                                  }).toList(),
