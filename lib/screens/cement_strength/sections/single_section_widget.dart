// filepath: lib/screens/cement_strength/sections/single_section_widget.dart
...
class CementStrengthSingleSectionWidget extends StatelessWidget {
  ...
  const CementStrengthSingleSectionWidget({super.key, required this.featuresMeta, required this.controllers, required this.formKey});
  ...
  void _submit(BuildContext context) {
    ...
    for (final k in featuresMeta.keys) {
      ...
      if (featuresMeta[k]!['type'] == 'string') {
        payload[k] = val;
      } else if (featuresMeta[k]!['type'] == 'int') {
        payload[k] = val.isEmpty ? null : int.tryParse(val);
      } else {
        payload[k] = val.isEmpty ? null : double.tryParse(val) ?? val;
      }
    }
    context.read<CementStrengthBloc>().add(PredictSingleEvent(payload));
  }
